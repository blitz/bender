...
_Noreturn void jmp_multiboot(const void *mbi, uint32_t entry)
...
static inline void outb(const uint16_t port, uint8_t value) {
  asm volatile("outb %0,%1" ::"a"(value), "Nd"(port));
}

/* TODO Wrong order (w/regard to the Linux source) */
static inline void outw(const uint16_t port, uint16_t value) {
  asm volatile("outw %0,%1" ::"a"(value), "Nd"(port));
}

static inline void outl(const uint16_t port, uint32_t value) {
  asm volatile("outl %0,%1" ::"a"(value), "Nd"(port));
}

static _Noreturn inline void cli_halt(void) {
  while (1) {
