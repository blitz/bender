...
#define PCI_ITER_INIT                                                          \
  { .bus_device = 0, .function = 0, .function = 0 }

/// Iterate to the next PCI device.
///
/// Returns true, if a device was found. In this case, *dev will be filled with
/// device info.
bool pci_iter_next_device(struct pci_iter *iter, struct pci_device *dev);

/// Similar to pci_iter_next_device, but finds the next device that matches a
/// predicate.
...
///
/// Returns true, if a PIO BAR is found. In this case, *iobase contains the port
/// number.
