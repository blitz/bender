...
void serial_init(void) {
  // We could check whether COM1 is actually available via the equipment word in
  // the BIOS Data Area, but this has not been reliable. So now we just blindly
  // configure the port and hope for the best.
...
void serial_init_port(uint16_t base_port) {
  ...
  outb(serial_base + FCR,
       FCR_FIFO_ENABLE | FCR_RECV_FIFO_RESET | FCR_TMIT_FIFO_RESET);
  outb(serial_base + MCR, MCR_DTR | MCR_RTS);
}

void serial_disable(void) { output_enabled = false; }
